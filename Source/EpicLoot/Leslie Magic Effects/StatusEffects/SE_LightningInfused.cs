...
namespace EpicLootLeslieAlphaTest.src.StatusEffects
{
    public class SE_LightningInfused : SE_Stats
    {
        ...
        private float infusedLightningDamage = 0f;
        ...
        public override void Setup(Character character)
        {
            ...
            m_icon = ObjectDB.instance.GetItemPrefab("SwordNiedhoggLightning")?.GetComponent<ItemDrop>()?.m_itemData.m_shared.m_icons[0];
            m_name = "Lightning Infused";
            m_ttl = 120f;
            ...
            Jotunn.Logger.LogWarning($"[LightningInfused] weapon base damage:{waeponBaseDmg} efficacy:{Efficacy} weapon multiplier:{realMeleeMultiplier} added:{waeponBaseDmg * Player.m_localPlayer.GetTotalActiveMagicEffectValue("Infusion", .01f)} + {Player.m_localPlayer.GetTotalActiveMagicEffectValue("Infusion", .01f) * Efficacy} * {realMeleeMultiplier}");
            infusedLightningDamage += ((waeponBaseDmg * Player.m_localPlayer.GetTotalActiveMagicEffectValue("Infusion", .01f)) + (Efficacy * Player.m_localPlayer.GetTotalActiveMagicEffectValue("Infusion", .1f)) * realMeleeMultiplier);
            weapon.m_shared.m_damages.m_lightning += infusedLightningDamage;

            Jotunn.Logger.LogWarning($"Added {infusedLightningDamage} lightning damage");
        }
        ...
        private void CreateVFX()
        {
            ...
            {
                m_vfxInstance = Object.Instantiate(InfusionVFX.LightningInfusionVFX, vis.m_rightHand);
                m_vfxInstance.SetActive(true);
            }
            else
            {
                m_vfxInstance = Object.Instantiate(InfusionVFX.LightningInfusionVFX, vis.m_rightHand);
                m_vfxInstance2 = Object.Instantiate(InfusionVFX.LightningInfusionVFX, vis.m_leftHand);
                m_vfxInstance.SetActive(true);
