...
namespace EpicLootLeslieAlphaTest.src.StatusEffects
{
    public class SE_FireInfused : SE_Stats
    {
        ...
        private float infusedFireDamage = 0f;
        ...
        public override void Setup(Character character)
        {
            ...
            m_icon = ObjectDB.instance.GetItemPrefab("SwordDyrnwyn")?.GetComponent<ItemDrop>()?.m_itemData.m_shared.m_icons[0];
            m_name = "Fire Infused";
            m_ttl = 120f;
            ...
            Jotunn.Logger.LogWarning($"[FireInfused] weapon base damage:{waeponBaseDmg} efficacy:{Efficacy} weapon multiplier:{realMeleeMultiplier} added:{waeponBaseDmg * Player.m_localPlayer.GetTotalActiveMagicEffectValue("Infusion", .01f)} + {Player.m_localPlayer.GetTotalActiveMagicEffectValue("Infusion", .01f) * Efficacy} * {realMeleeMultiplier}");
            infusedFireDamage += ((waeponBaseDmg * Player.m_localPlayer.GetTotalActiveMagicEffectValue("Infusion", .01f)) + (Efficacy * Player.m_localPlayer.GetTotalActiveMagicEffectValue("Infusion", .1f)) * realMeleeMultiplier);
            weapon.m_shared.m_damages.m_fire += infusedFireDamage;

            Jotunn.Logger.LogWarning($"Added {infusedFireDamage} fire damage");
        }
        ...
        private void CreateVFX()
        {
            ...
            {
                m_vfxInstance = Object.Instantiate(InfusionVFX.FireInfusionVFX, vis.m_rightHand);
                m_vfxInstance.SetActive(true);
            }
            else
            {
                m_vfxInstance = Object.Instantiate(InfusionVFX.FireInfusionVFX, vis.m_rightHand);
                m_vfxInstance2 = Object.Instantiate(InfusionVFX.FireInfusionVFX, vis.m_leftHand);
                m_vfxInstance.SetActive(true);
